...
Our results suggest that \algname is accurate to predict Math assessments (Correlation=0.55 , Spearman $\rho$=0.51).
\end{abstract}
...
Moreover, using a fairly straight forward data-driven process to replace the labor intensive task of carefully designing a game-specific model (by experts), makes \algname a good candidate for modeling learning at scale. 
...
Shute et al. \cite{shute2013stealth} studied the effect of ECD design on student learning and found that students who played the game in a 4 hour session, showed significant improvement in their qualitative, conceptual physics understanding.
...
The time complexity of DBSCAN is $O(n \log n)$ (n = the number of data points) and there are recent parallelized implementations that are failry scalable \cite{patwary2012new,dai2012efficient}.
...
We hypothesized that students in difference performance groups play the game differently. Our sequence modeling phase should be able to capture this difference by observing the playing patterns of each group.
...
For each student $s$  in each game level $g$, we calculate the difference between ($d_{s,g}$) of the likelihood of belonging to high performing group and the likelihood of of belonging to the low performing group.
We calculate this likelihood by estimating the Forward-Backward probabilities  on each student sequence sequence of actions  based on the two HMMs parameter we estimated (whether the student is in the high performing group or in the low performing group) and using a hyperbolic tangent function in order to convert it to a value between -1 and 1: 
\begin{equation}
...
However, due to the design of our experiment (predicting students' post-test scores) and the fact that there is only one post-test score for each student in our dataset, we decided to  only incorporate the first attempt (sequence of actions) in each game level in our analysis.
...
Moreover, the correlation and Spearman $\rho$ of our predicted values with the true values was much higher than both baselines. 
...
\begin{table}[tbh]
	...
	\begin{tabular}{@{}lllll@{}}
		...
		Success / Failure                                                      & 0.02                                                     & 0.16                                                 & 3.21                                                          & 4.08 \\
				...
				3.60 & 3.10 \\ 
		\textbf{\algname ($K=2, H>1$)}                                                      & \textbf{0.55}                                                     & \textbf{0.51}                                                 & \textbf{2.84}                                                          & \textbf{3.35} \\ \bottomrule
	\end{tabular}
	\caption{The results of predicting post-test scores using three different feature sets.}~\label{tab:results}	
\end{table}

In Table \ref{tab:results} we reported two versions of the \algname algorithm, one with $H=1$ and one with $H>1$. 
The HMM with only one state ($H=1$) represents the model with no  memory that only considers the distribution of different activities in a sequence (similar to Gaussian Mixture Models).
The best performing model ($H>1$) however, learns the best number of states for each game level based on the training examples and outperforms the model with only one state.
...
Unfortunately, the improvement was not significant ($p=0.20$ the Success/Failure baseline and $p=0.26$ the Sequence Length baseline).
...
We believe this observation is mainly happened due to the fact that finding the solution to this game level requires mastery in all three major subskills targetted by the game design.
%Maybe Kristen is the best person to shed some lights on this observation.
...
In order to avoid using information from the independant variable to calculate the features of our regression model, we are using a \%80-\%20 split to validate the results instead of for example a 5-fold cross validation.
