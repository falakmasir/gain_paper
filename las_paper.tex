...
\begin{document}
	...
	\title{\plaintitle}
	% \alignauthor{\mbox{\hspace{-4em}Mohammad H. Falakmasir$^{\star, \#} $\;\; Jos\'{e} P. Gonz\'{a}lez-Brenes$^\star$ \;\; Geoffrey J. Gordon$^\S$ \;\; Kristen E. DiCerbo$^\star$}\\
	\numberofauthors{4}
	\author
	{%
		\alignauthor{Mohammad Falakmasir\titlenote{Part of the research  was conducted while the author visited Pearson.}\\
			\affaddr{Intelligent Systems Program} \\
			\affaddr{University of Pittsburgh}\\
			\email{falakmasir@pitt.edu}\\
		}
		\alignauthor{Jos\'{e} P. Gonz\'{a}lez-Brenes\\
			\affaddr{School Research}\\
			\affaddr{Pearson}\\
			\email{jose.gonzalez-brenes@pearson.com }\\
		}
		\and
		\alignauthor{ Geoffrey J. Gordon\\
			\affaddr{Machine Learning}\\
			\affaddr{Carnegie Mellon University} 
			\email{ggordon@cs.cmu.edu}\\
		}
		\alignauthor{Kristen E. DiCerbo\\
			\affaddr{School Research}\\
			\affaddr{Pearson}\\
	...
	\conferenceinfo{L@S 2016,}{April 25-26, 2016, Edinburgh, Scotland Uk}
	\isbn{978-1-4503-3726-7/16/04}\acmPrice{\$15.00}
	...
	%,
	%by including an evidence-centered assessment design and formative feedback to support learning.
	...
	%A wide range of machine learning problems in different domains (e.g. speech and language processing, biology, and vision) involve mapping sequences of inputs into some type of output.
	%The goal is to find a representation that can be used for describing, visualizing, and finding (dis)similarities between sequences.
	%Hidden Markov Models (HMMs) provide a probabilistic framework for modeling sequence data that has been successfully adapted in a wide range of applications~\cite{rabiner1989tutorial}.
	%However, they are often restricted to discrete or Gaussian observations and the latent state variable is 
	...
	\begin{comment}
	as input and generates a sequence of abstract observations by employing an unsupervised clustering method. 
	...
	We assume that high- and low-performing students interact differently with the game environment.
	...
	The HMM with only one state ($H=1$) represents the model with no memory that only considers the distribution of different observations in a sequence (as a Gaussian Mixture Models) in order to demonstrate the effect of our discretization step.
	...
	The correlation between true and predicted values is only statistically significant for our best-performing model, ($r=0.55$, $n=15$, and $p < 0.05$), but the other models show no statistical significance.
	This suggests that the predicted scores of \algname ($K=2$) are positively correlated with the true assessment scores.
	In order to evaluate the significance of improvements, we also calculated the absolute difference between the true and predicted post-test scores for all of the students based on each model.
	We used a paired one-sided t-test (n=15) for judging the significance of improvement over the baselines.
	However, we cannot reject the null hypothesis that the baselines have greater or equal MAE than \algname ($p=0.20$ over the Success/Failure baseline and $p=0.26$ over the Sequence Length baseline).
