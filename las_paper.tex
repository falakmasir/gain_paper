...
Our sequence modeling phase should be able to capture this difference by capturing the interaction patterns of each group in different game levels.
...
\textit {Alice in AreaLand} is an educational game developed as a Pearson Insight Learning System \footnote{http://researchnetwork.pearson.com/learning-science/insight-learning-system}. 
...
The HMM with only one state ($H=1$) represents the model with one state that only considers the distribution of different activities in a sequence (similar to Gaussian Mixture Models) in order to demonstrate the effect of our discretization step.
...
The difference in predictive performance between games is likely related to game design. 
Some games are designed to introduce game tools and techniques while others are created to present choices to players aligned with math proficiency.
