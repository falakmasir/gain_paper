...
<<<<<<< Updated upstream
	...
	\algname allows  combining data collected from students playing more than a game.
	For each game, the discretization step transforms slot and filler observations into multinomial indicators that are used as evidence for learning.
	Then, the sequence modeling step learns a representation from data in the form of a Hidden Markov Model for different group of students (e.g, high- and low-performing) per game.
	The regression step combines the evidence of the multiple games.
	This step uses the likelihood of student sequences in each game level as a feature to predict the post-test results.
=======
	...
	Algorithm~\ref{alg:spring} describes the three main steps of our pipeline: discretization,  sequence modeling, and feature vector modeling.
...
>>>>>>> Stashed changes
	...
	Algorithm~\ref{alg:discretize} describes the method we use to discretize fillers for each game.
	...
	Then, for each student $s$ in each game level $g$, we can calculate the difference ($d_{s,g}$)  between of the likelihood of belonging to the high- and low-performing group.
	...
	\hl{Where do you define what K is?? How did you tune it??}
	
	We can use  a linear regression model that predicts the post-test scores from this difference:
	\begin{equation}
...
<<<<<<< Updated upstream
	...
	To learn a \algname model, we need training data of assessment scores.
	For this reason, we only use the interactions of the students who also participated in the post-test.
	In our implementation, we consider each game level as a different game. 
	This is reasonable, because the rules and objectives vary considerably.
	Additionally, the types of slots and fillers vary through game levels.
	
	%Different attempts a student makes at a problem and the change of performance over time are a rich data source for inferring proficiency.
	Due to the design of our experiment (predicting students' post-test scores) and the fact that there is only one post-test score for each student in our dataset, we decided to  only incorporate the first attempt (sequence of actions) in each game level in our analysis. 
	\hl{I was not able to understand the previous sentence.  Grammar is unclear, and I don't know what an ``attempt" is}
		
=======
>>>>>>> Stashed changes
	\subsection{Experimental Setup}
	\hl{You say that there are two groups, training and test, and then you mention a development set. Technical writing requires precision}
	...
	\begin{itemize}
		...
		It is a model with no memory (no temporal information), because  it only consider the distribution of different observations in a sequence (as a Gaussian Mixture Models).
		This model evaluates  the effect of our discretization step. \hl{Why?}
