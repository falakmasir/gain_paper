...
We present a new machine learning analysis pipeline, {Student Profficiency Inferrer from Game data} (\algname), that allows modeling game playing behavior in educational games.
...
{}
\keywords{Educational Games, Unsupervised Learning, Invisible Assessment}
...
An interesting alternative to traditional (summative) assessment is  invisible assessment~\cite{shute2013stealth},
...
%The ECD framework provides the ability to collect activity stream data as individuals interact with digital artifacts and provides a record of what students are doing as they progress through learning activities.
%This data can be modeled provide information about learning processes and has the potential to greatly reduce our need to subsequently deliver separate, discrete tests for the sole purpose of formatively measuring student proficiency.



% I generate tables with:  http://www.tablesgenerator.com/ --- Thanks!
...
For example, prior work~\cite{shute2013stealth, shute2009melding} has relied extensively on subject matter expertise to define features for a regression model \hl{confirm if it's regression}
\hl{give examples of the things they did manually}.

Invisible assessment from game data may become more accurate and cheaper to implement if the domain knowledge engineering could be automated by a data-driven process.
...
Unfortunately, generic machine learning algorithms input \textit{feature vectors} or \textit{sequences} and not slot and filler structures.
\hl{Describe what are feature-vectors methods and sequences}.
Traditional machine learning algorithms, and off-the-shelf data mining packages, such as Weka~\hl{(cite)}, cannot help use slot and filler data because
...
In this paper, we propose \textit{Student PRofficiency INferrer from Game data} (SPRING), a new machine learning analysis pipeline that models game playing behavior.
...
\EndIf
\end{algorithmic}
\caption{The \algname algorithm. \label{alg:spring}}
\end{algorithm}

% You can find documentation here: https://en.wikibooks.org/wiki/LaTeX/Algorithms
...
For example, for each movement there is a two dimensional coordinates of the target $x$ and $y$ and for each instance of using the gluumi tool \hl{do not talk about Alice in WL here?}, there is a variable that represents the size of the new object that has been created. 
...
In order to make sure that the model is generalizable across multiple game levels, we first used DBSCAN \hl{DBScan is an example, not the only way to do this} clustering algorithm \cite{ester1996density} on the $x$ and $y$ coordinates of the movements to extract the crucial regions within the game interface.
Then we transformed each movement into its cluster number and considered outliers as a separate cluster.
\hl{ use images for examples} 
...
Finally, in the \hl{verification} phase, we test the regression model on the held-out set (\%20) and report the results.
