...
An interesting alternative to traditional (summative) assessment is stealth assessment~\cite{shute2013stealth},
...
Stealth assessment is also intended to remove (or seriously reduce) test anxiety, while not sacrificing validity and reliability \cite{shute2008you}.
...
For example, prior work~\cite{shute2013stealth, shute2009melding} has relied extensively on subject matter expertise to define a student model in form of a  bayesian network (competency model) and extract features of the performance data related to each competency (evidence model).
...
\begin{algorithm}
\begin{algorithmic}
\State Extract Clusters from 2D movements (x, y)
\State Use Cluster IDs to transform movements into multinomial variables
\State Use Converted Trajectories to Learn the State Sequence for High- and Low-Performing Students
...
\State Predict Student Post-Test Scores
\end{algorithmic}
...
\subsection{Results}
In order to evaluate our regression model we decided to compare its performance agains two baselines: 1) A regression model that uses success and failure of students in each game level as feature and 2) A regression model that uses the sequence lengths in each game level. Our model was able to significantly ourperform both baselines regarding to mean absolute errot and root mean squared error. Moreover, the $R^2$ correlation and Spearman $\rho$ of our predicted values with the true values was much higher than both baselines. Table \ref{tab:results} shows the results.
