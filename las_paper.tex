...
\begin{document}
	...
	\item Sequence Length: A regression model that uses the normalized  \hl{What is normalized?} sequence lengths in each game level. 
	\end{itemize}
	
		
	Table \ref{tab:results} summarizes our results.
	We used each method to predict the students in the test set's  scores. 
	...
	We cannot reject the null hypothesis that the baselines outperform SPRING \algname ($p=0.20$ over the Success/Failure baseline and $p=0.26$ over the Sequence Length baseline).
