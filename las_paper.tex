...
In this section, we describe  the discretization, sequence modeling and regression steps of \algname
%Procedure~\ref{alg:spring} describes the SPRING framework.
...
\begin{figure}
	...
	\includegraphics[width=0.9\columnwidth]{figures/glacier}
		\caption{The clusters detected for game level 2, \textit {None Shall Pass!}, using DBSCAN method. We transform each movement action into corresponding cluster number during the discretization step.}~\label{fig:figurecluster}
\end{figure}

We replace each (slot, filler) tuple with its corresponding clusterID. Procedure~\ref{alg:discretize} shows the steps.
...
\begin{algorithm}
	\begin{algorithmic}
		...
		%				
		%	
		%	\Procedure{LearnModel}{S}
		%		\State{D, P = Discretize(A,S)}
		%		\State{Low, High = TrainHMM(S,C)}
		%		\State \Return{Low, High}
		%	\EndProcedure
