...
\begin{abstract}
We present \textit{Student PRofficiency INferrer from Game data} (\algname), a novel machine learning algorithm that models game playing behavior. 
We demonstrate \algname with data collected from students playing a collection of 12 educational mini-games.
Our experiments suggests that \algname is a useful method to predict student test scores ($R^2$ =0.55 , Spearman $\rho$=0.82). 
Moreover, we provide insights in how to use \algname to understand student exploration habits, misconceptions, and how to improve the game design based on their playing strategies.
\end{abstract}
...
Assessments are important because they collect evidence about student knowledge.
